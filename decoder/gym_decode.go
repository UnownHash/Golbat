...
func (gym *Gym) updateGymFromFort(fortData *pogo.PokemonFortProto, cellId uint64, timestampMs int64) *Gym {
	...
	if fortData.RaidInfo != nil {
		...
		if fortData.RaidInfo.RaidPokemon != nil {
			...
			gym.SetRaidPokemonStamina(null.IntFrom(int64(fortData.RaidInfo.RaidPokemon.MaxStamina)))
			gym.SetRaidPokemonCpMultiplier(null.FloatFrom(float64(fortData.RaidInfo.RaidPokemon.CpMultiplier)))
			...
			gym.SetRaidPokemonStamina(null.NewInt(0, false))
			gym.SetRaidPokemonCpMultiplier(null.NewFloat(0, false))

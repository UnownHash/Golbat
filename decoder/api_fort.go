...
func GymScanEndpoint(retrieveParameters ApiFortScan, db db.DbDetails) *ApiGymScanResult {
	...
	for _, key := range returnKeys {
		gym, err := GetGymRecord(context.Background(), db, key)
		if err == nil && gym != nil {
			results = append(results, gym)
		}
	}
...
func PokestopScanEndpoint(retrieveParameters ApiFortScan, db db.DbDetails) *ApiPokestopScanResult {
	returnKeys, examined, skipped, total := internalGetForts(POKESTOP, retrieveParameters)
	results := make([]*Pokestop, 0, len(returnKeys))
	start := time.Now()

	for _, key := range returnKeys {
		pokestop, err := GetPokestopRecord(context.Background(), db, key)

...
func TestEncounterStatsDurationKeepsDedupWindow(t *testing.T) {
	...
	past := &Pokemon{PokemonData: PokemonData{
		...
		ExpireTimestamp:         null.ValueFrom(uint32(now - 300)),
	}}
	...
	future := &Pokemon{PokemonData: PokemonData{
		ExpireTimestampVerified: true,
		ExpireTimestamp:         null.ValueFrom(uint32(now + 600)),
	}}
...
func TestRemainingDurationVerifiedBoundary(t *testing.T) {
	...
	atBoundary := &Pokemon{PokemonData: PokemonData{ExpireTimestampVerified: true, ExpireTimestamp: null.ValueFrom(uint32(now))}}
	if got := atBoundary.remainingDuration(now); got != time.Minute {
		t.Errorf("timeLeft=60 → %v, want 1m", got)
	}
	justOver := &Pokemon{PokemonData: PokemonData{ExpireTimestampVerified: true, ExpireTimestamp: null.ValueFrom(uint32(now + 1))}}

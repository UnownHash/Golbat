...
import (
	...
	"golbat/external"
	golbat_grpc "golbat/grpc"
	"golbat/http_handler"
	"golbat/proto_decoder"
...
func main() {
	...
	protoDecoder := func() *proto_decoder.ProtoDecoder {
		timeout := 5 * time.Second
		if cfg.Tuning.ExtendedTimeout {
			timeout = 30 * time.Second
		}
		return proto_decoder.NewProtoDecoder(timeout, dbDetails, statsCollector, deviceTracker)
	}()

	httpHandler := http_handler.NewHTTPHandler(
		http_raw_decoder.NewHTTPRawDecoder(protoDecoder),
		dbDetails,
		statsCollector,
		deviceTracker,
	)
	...
	if cfg.GrpcPort > 0 {
		...
		grpcRawDecoder := grpc_raw_decoder.NewGRPCRawDecoder(protoDecoder)
		go func() {
			...
			s := grpc.NewServer()
			golbat_grpc.RegisterRawProtoServer(s, &grpcRawServer{rawDecoder: grpcRawDecoder})
			golbat_grpc.RegisterPokemonServer(s, &grpcPokemonServer{})
	...
	apiGroup.GET("/devices/all", httpHandler.GetAllDevices)

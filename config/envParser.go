...
// Copied from https://github.com/knadh/koanf/blob/98b36ee02993f8a0632e4a2ec9496be066936133/providers/env/env.go
// And modified to support slices of structs
...
type Env struct {
	prefix string
	delim  string
	cb     func(key string, value string, mp map[string]interface{}) (string, interface{})
}
...
func Provider(prefix, delim string, cb func(s string) string) *Env {
	...
	if cb != nil {
		e.cb = func(key string, value string, _mp map[string]interface{}) (string, interface{}) {
			return cb(key), value
		}
	}
	return e
...
func ProviderWithValue(prefix, delim string, cb func(key string, value string, _mp map[string]interface{}) (string, interface{})) *Env {
	return &Env{
		prefix: prefix,
		delim:  delim,
		cb:     cb,
...
func (e *Env) Read() (map[string]interface{}, error) {
	...
	// sort to guarantee slices elements order
	sort.Strings(keys)
	...
	for _, k := range keys {
		...
		if e.cb != nil {
			key, value := e.cb(parts[0], parts[1], mp)

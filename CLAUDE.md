...
Golbat supports two protobuf decoders selected by method type via `[proto_engine]` config:

**Standard decoder** (`"std"`): Original stable decoder used by all paths.

**Hyperpb decoder** (`"hyperpb"`): High-performance arena-allocated decoder. Generated by `cmd/pogoshimgen` via `scripts/genshim.sh` after `vbase.pb.go` updates. Arena-allocated message access is faster (cache-friendly layout, reduced allocations) but requires careful lifetime management:
...
- Payload bytes are owned by the arena and freed when `decodeWithArena()` returns. Getters that need persistence copy into independently-allocated structures.
- The `pogoshim` generated file is several hundred KB and treated as generated code (spot-check only on review).

**Engine selection** (`[proto_engine]` config):
- `gmo` (GetMapObjects), `encounter`, `disk_encounter`: which decoder to use for each method.
- Default: all `"hyperpb"`.
- **Rollback**: Flip `gmo = "std"` (etc.) in config and restart — no rebuild needed.
